...
router.post('/auth', (req, res) => {
  ...
  if (!user) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).run(tgUser.id, tgUser.username || null, tgUser.first_name || null, tgUser.last_name || null, tgUser.photo_url || null, tgUser.is_premium ? 1 : 0);

    db.prepare(`INSERT OR IGNORE INTO owned_skins (telegram_id, skin_id) VALUES (?, 'default')`).run(tgUser.id);
    ...
    db.prepare(`
      ...
      WHERE telegram_id = ?
    `).run(tgUser.username || user.username, tgUser.first_name || user.first_name, tgUser.last_name || user.last_name, tgUser.photo_url || user.photo_url, tgUser.is_premium ? 1 : 0, tgUser.id);
  }
  ...
  const ownedSkins = db.prepare('SELECT skin_id FROM owned_skins WHERE telegram_id = ?').all(tgUser.id).map(r => r.skin_id);
  const powerUps = {};
  ...
  res.json({
    user: {
      ...
      lastName: user.last_name, photoUrl: user.photo_url, isPremium: !!user.is_premium,
      energy: user.energy, maxEnergy: MAX_ENERGY, lastEnergyAt: user.last_energy_at,
      energyRegenMs: ENERGY_REGEN_MS, coins: user.coins, bestScore: user.best_score,
      totalGames: user.total_games, equippedSkin: user.equipped_skin,
      vipExpiry: user.vip_expiry, unlimitedExpiry: user.unlimited_expiry,
      ownedSkins, powerUps, isUnlimited: isUnlimited(user),
    }
  });
});
...
router.get('/profile', (req, res) => {
  ...
  const ownedSkins = db.prepare('SELECT skin_id FROM owned_skins WHERE telegram_id = ?').all(user.telegram_id).map(r => r.skin_id);
  const powerUps = {};
  const puRows = db.prepare('SELECT type, quantity FROM powerups WHERE telegram_id = ?').all(user.telegram_id);
  for (const r of puRows) powerUps[r.type] = r.quantity;

  res.json({
    energy: regen.energy, maxEnergy: MAX_ENERGY, lastEnergyAt: regen.lastEnergyAt,
    coins: user.coins, bestScore: user.best_score, totalGames: user.total_games,
    equippedSkin: user.equipped_skin, vipExpiry: user.vip_expiry,
    unlimitedExpiry: user.unlimited_expiry, ownedSkins, powerUps, isUnlimited: isUnlimited(user),
  });
});
